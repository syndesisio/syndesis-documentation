...
[[style-code-callouts]]
=== Code Listing That Has Callouts and Replaceables
...
References to the Fuse installation directory must be:
...
To format references to the Fuse installation directory, enter
the following:
----
`_FUSE_HOME_`
----
...
`subs="+quotes"` just before the code block. For example:
  ...
  [subs="+quotes"]
  ----
  $ _FUSE_HOME_/bin/standalone.sh -c _CONFIG_FILE_.xml
  ----
